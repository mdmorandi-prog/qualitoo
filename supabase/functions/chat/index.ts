...
async function fetchSystemContext(supabaseAdmin: any) {
  ...
  const queries = [
    ...
    { key: "document_workflow_steps", query: supabaseAdmin.from("workflow_approval_requests").select("record_id, step_name, step_type, status, approver_role, decided_at").eq("module", "quality_documents").order("created_at", { ascending: false }).limit(30) },
...
- **Grupos de acesso**: definem acesso por setor com níveis (read/write/admin) **e permissões granulares por ação** (\`permission_actions\`: consultar, criar, editar, excluir, aprovar, exportar), com expiração opcional. O front consulta as ações pelo hook \`usePermissions\` (RPC \`get_user_actions\`) e o banco valida via \`has_action_permission(user, setor, ação)\`.
- **Workflow unificado**: as etapas de aprovação de documentos foram migradas do antigo \`document_workflow_steps\` para o motor genérico \`workflow_approval_requests\` (module = "quality_documents"), o mesmo usado por NCs, CAPAs e mudanças.
- **Portal do Fornecedor endurecido**: tokens de 256 bits com **expiração forçada em 7 dias**, limite de **30 acessos por hora** por token (rate limit) e **trilha completa de acessos** em \`supplier_portal_access_log\` (data, IP, user agent, ação, bloqueios por limite). Validação e registro ocorrem na Edge Function \`supplier-portal-access\`.
- **Assinatura com certificado ICP-Brasil (opcional)**: no diálogo de assinatura é possível anexar o certificado público (.cer/.crt/.pem). O sistema lê o X.509, valida a vigência, identifica a cadeia ICP-Brasil (MP 2.200-2) e grava titular, emissor, série, CPF/CNPJ, validade e política em \`document_signatures\`, marcando \`signature_type = icp_brasil\`. Sem certificado, a assinatura permanece eletrônica com hash SHA-256.
